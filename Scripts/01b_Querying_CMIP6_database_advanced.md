Querying CMIP6 database advanced
================
...
Just like in the notebook containing a [simple querying
example](01a_Querying_CMIP6_database_simple.md), we will use modified
functions from the
[epwshiftr](https://github.com/ideas-lab-nus/epwshiftr) package to look
for Coupled Model Intercomparison Project Phase 6 (CMIP6) data from the
Earth System Grid Federation
([ESGF](https://esgf.nci.org.au/search/cmip6-nci/)) platform.

This example provides some more advanced filtering to personalise your
query results. This notebook will show you how to look for models that
contain data from ocean, sea ice and biogeochemical variables. Further,
we will also check the [`ESGF` nodes
status](https://aims2.llnl.gov/nodes) to keep query results for active
nodes only.

For useful resources that will help you with your CMIP6 data query,
refer to the first section of the [simple querying
example](01a_Querying_CMIP6_database_simple.md).

## Loading libraries

As in the [simple querying
example](01a_Querying_CMIP6_database_simple.md), we will load the
modified functions from the `epwshiftr` package, which are on the
accompanying `search_download_functions` script. Additionally, we will
load the `tidyverse` and `lubridate` packages to filter out our query
results further and save these results.
...
As previously explained, we will be looking for variables from the
ocean, sea ice and biogeochemical models: sea surface temperature
(`tos`), sea ice concentration (`siconc`) and primary production
(`intpp`).
...
```
    ...
    ##                                                                                                                                                        file_id
    ## 1:    CMIP6.CMIP.CSIRO-ARCCSS.ACCESS-CM2.historical.r1i1p1f1.Omon.tos.gn.v20191108.tos_Omon_ACCESS-CM2_historical_r1i1p1f1_gn_185001-201412.nc|esgf.nci.org.au
    ## 2: CMIP6.CMIP.CSIRO.ACCESS-ESM1-5.historical.r1i1p1f1.Omon.intpp.gn.v20191115.intpp_Omon_ACCESS-ESM1-5_historical_r1i1p1f1_gn_185001-201412.nc|esgf.nci.org.au
    ## 3:     CMIP6.CMIP.CSIRO.ACCESS-ESM1-5.historical.r1i1p1f1.Omon.tos.gn.v20191115.tos_Omon_ACCESS-ESM1-5_historical_r1i1p1f1_gn_185001-201412.nc|esgf.nci.org.au
    ##                                                                                      dataset_id
    ## 1: CMIP6.CMIP.CSIRO-ARCCSS.ACCESS-CM2.historical.r1i1p1f1.Omon.tos.gn.v20191108|esgf.nci.org.au
    ## 2:   CMIP6.CMIP.CSIRO.ACCESS-ESM1-5.historical.r1i1p1f1.Omon.intpp.gn.v20191115|esgf.nci.org.au
    ## 3:     CMIP6.CMIP.CSIRO.ACCESS-ESM1-5.historical.r1i1p1f1.Omon.tos.gn.v20191115|esgf.nci.org.au
    ##    mip_era activity_drs institution_id     source_id experiment_id member_id
    ## 1:   CMIP6         CMIP   CSIRO-ARCCSS    ACCESS-CM2    historical  r1i1p1f1
    ## 2:   CMIP6         CMIP          CSIRO ACCESS-ESM1-5    historical  r1i1p1f1
    ## 3:   CMIP6         CMIP          CSIRO ACCESS-ESM1-5    historical  r1i1p1f1
    ##    table_id frequency grid_label  version nominal_resolution variable_id
    ## 1:     Omon       mon         gn 20191108             250 km         tos
    ## 2:     Omon       mon         gn 20191115             250 km       intpp
    ## 3:     Omon       mon         gn 20191115             250 km         tos
    ##                                                 variable_long_name
    ...
    ## 1:           degC     1850-01-01   2014-12-01 489195464 esgf.nci.org.au
    ## 2:    mol m-2 s-1     1850-01-01   2014-12-01 500537777 esgf.nci.org.au
    ## 3:           degC     1850-01-01   2014-12-01 490332982 esgf.nci.org.au
    ##                                                                                                                                                                                        file_url
    ## 1:    http://esgf.nci.org.au/thredds/fileServer/master/CMIP6/CMIP/CSIRO-ARCCSS/ACCESS-CM2/historical/r1i1p1f1/Omon/tos/gn/v20191108/tos_Omon_ACCESS-CM2_historical_r1i1p1f1_gn_185001-201412.nc
    ## 2: http://esgf.nci.org.au/thredds/fileServer/master/CMIP6/CMIP/CSIRO/ACCESS-ESM1-5/historical/r1i1p1f1/Omon/intpp/gn/v20191115/intpp_Omon_ACCESS-ESM1-5_historical_r1i1p1f1_gn_185001-201412.nc
    ## 3:     http://esgf.nci.org.au/thredds/fileServer/master/CMIP6/CMIP/CSIRO/ACCESS-ESM1-5/historical/r1i1p1f1/Omon/tos/gn/v20191115/tos_Omon_ACCESS-ESM1-5_historical_r1i1p1f1_gn_185001-201412.nc
    ##                                          dataset_pid
    ## 1: hdl:21.14100/bcf7111a-f2e9-320c-aa60-308c74c34f13
    ## 2: hdl:21.14100/311d0772-ce1e-3440-8745-85eb365c65e0
    ## 3: hdl:21.14100/f485bfb8-0aa6-3ba1-a413-9ffa4a2e9ef2
    ##                                          tracking_id
    ## 1: hdl:21.14100/0bcaaa74-aedb-4d45-a5e5-cb3ab467f2b5
    ## 2: hdl:21.14100/44046366-ac50-4be5-bf42-e1be7c65e37e
    ## 3: hdl:21.14100/02850fcc-be64-40de-b7ca-9b8aa6e688a0

### Reviewing and tidying up search results

First, we will provide the file path to the folder where we will save
our data. Note that you must update this path to a folder where you
would like to save the CMIP6 files that contain the data you are
interested in.
...
Since we have over 5,000 matches for our query, we will apply a series
of conditions that will help us identify the files we are interested in.
The filters applied are as follows:  
1. Model outputs up to the year 2100  
2. Files containing data from the first and last decade of the
model/experiment  
3. Model outputs from nodes that are currently active  
4. Models that contain all three variables

Additionally, we will add a few extra columns into our results data
frame that will help us automate the extraction and storage of the data
we need.
...
then only keep datasets up to the year 2100.
...
The total number of relevant outputs decreased slightly, but we still
have over 5,000 results. In the next chunk we will identify the models
that offer data for the three variables we are interested in: sea
surface temperature (`tos`), sea ice concentration (`siconc`) and
primary production (`intpp`). Models that include only one or two of
these variables will be removed from our results.
