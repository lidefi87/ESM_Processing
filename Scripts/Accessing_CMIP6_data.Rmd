...
ds = ds.sel(time = slice(str(dec_0), str(dec_0+9)))
#Calculating monthly means and plotting only the first month
ds.intpp.groupby('time.month').mean('time')[0].plot(levels = 9)
#Show plot
...
```{r}
#Loading raster
r <- nc_open(results_query$out_file_name[1])
#Accessing variable of interest
var <- ncvar_get(r, results_query$variable_id[1])
...
ds_dates <- time %>% 
  ...
  mutate(time = as_date(value, origin = origin),
         ...
         dec = case_when(year <= results_query$decade_0_end ~ 1,
                         T ~ 2)) %>% 
...
for(d in unique(ds_dates$dec)){
  ...
  dec_month_mean[[d]] <- array(unlist(month_mean), dim = c(length(x), length(y), 12))
  }
dec_time <- month.name
dec_time_units <- "month"

image(x, y, dec_month_mean[[1]][,,1], col=rev(RColorBrewer::brewer.pal(9,"YlGnBu")))
...
clim_ds <- ncvar_def("monthly_mean", ncatt_get(r, "intpp")$units,
                     list(xdim, ydim, timedim), ncatt_get(r, "intpp")$missing_value,
                     paste0("Monthly mean ", ncatt_get(r, "intpp")$long_name))
...
ncout <- nc_create("test1.nc", list(clim_ds, lon_ds, lat_ds), force_v4 = T)
ncvar_put(ncout, clim_ds, dec_month_mean[[1]])
ncvar_put(ncout, lon_ds, lon[,,1:12])
ncvar_put(ncout, lat_ds, lat[,,1:12])
for(i in 1:length(global_atts)){
  ncatt_put(ncout, 0, names(global_atts)[i], global_atts[[i]])
}
