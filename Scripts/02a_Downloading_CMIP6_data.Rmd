...
The `reticulate` package in `R` allow us to use `Python` within an `R` script. We will be using the `conda environment` defined in the `.Rprofile` file. In simple terms, a `conda environment` is a directory that contains all the `Pyhton` libraries and the dependencies needed to run this script. The `README` file in this repository has instructions on how to access this environment if you are using CLIOTOP, or how to install it if you are running these scripts in your own computer.
